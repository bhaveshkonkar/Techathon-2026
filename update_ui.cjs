...
// Wait, 'translate' becomes 'tranzinc' if we do blind replace!
code = code.replace(/tranzinc/g, 'translate'); // Revert the mistake
...
code = code.replace(/shadow-2xl shadow-emerald-500\/[0-9]+/g, 'shadow-xl shadow-black/50');
code = code.replace(/shadow-2xl shadow-emerald/g, 'shadow-xl shadow-black');
code = code.replace(/shadow-2xl /g, 'shadow-xl ');
code = code.replace(/shadow-xl shadow-black\/50/g, 'shadow-lg shadow-black/80');
...
code = code.replace(/bg-emerald-500/g, 'bg-white');
...
code = code.replace(/text-emerald-500/g, 'text-zinc-300'); // generic fallback
...
code = code.replace(/bg-zinc-950/g, 'bg-[#060606]');
...
code = code.replace(/ring-indigo-/g, 'ring-zinc-');
